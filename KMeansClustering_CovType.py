...
PCA_data = PCA(n_components = 3,whiten=False)
...
PCA_explained_variance=PCA_data.explained_variance_
print('\n\n\n', 'PCA explained variance: ',PCA_explained_variance)

PCA_explained_variance_ratio=PCA_data.explained_variance_ratio_
print('\n\n\n', 'PCA explained variance ratio: ',PCA_explained_variance_ratio)
...
import scipy
ICA_data = FastICA(n_components = 3)
...
transformer = GaussianRandomProjection(n_components=3,eps=0.00001)
...
transformer = LinearDiscriminantAnalysis(solver="svd",n_components = 3)
...
PCA_data_trans = PCA_data.fit_transform(X_train)
PCA_data_trans_test = PCA_data.fit_transform(X_test)
...
start_time=time()
######
##Apply neural network to one of the datasets with dimensionality reduced projected data considering these features as new features
from sklearn.neural_network import MLPClassifier
neuralnet = MLPClassifier(solver='adam', alpha=0.000001,learning_rate_init=0.001,max_iter=100,verbose=0,hidden_layer_sizes=(7, 3))
classifierdata=neuralnet.fit(X_train,y_train)
y_train_pred = neuralnet.predict(X_train)
